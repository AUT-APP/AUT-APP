...
data class Booking(
    ...
    val isActive: Boolean
        get() = status == BookingStatus.ACTIVE && Date().let { now -> now.after(startTime) && now.before(endTime) }

    val isUpcoming: Boolean
        get() = status == BookingStatus.ACTIVE && startTime.after(Date())

    val isCompleted: Boolean
        get() = status == BookingStatus.CANCELED || status == BookingStatus.COMPLETED || endTime.before(Date())
